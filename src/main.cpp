...
/* Matrices */
glm::vec3 cam_position = glm::vec3(0.0f, 8.5f, 17.0f);
glm::vec3 cam_look_at  = glm::vec3(0.0f, -35.0f, 0.0f);
...
glm::vec3 lightPos = glm::vec3(9.0f, 9.0f, 0.0f);
...
int init()
{
    ...
    window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Statue of Florian C.", nullptr, nullptr);
...
int loadContent()
{
    mesh = new Model("res/models/Florian_C.obj");
    ...
    texture = new Texture();
    texture->load("res/models/Florian_C_Material.png");
    texture->bind();
...
void render(float time)
{
    ...
    /* Draw our triangle */
    world_matrix = glm::rotate(glm::mat4(1.0f), glm::radians(rot_value), glm::vec3(0, 0, 1));
...
void update()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_A )) rot_value -= 1.5;
        else if (glfwGetKey(window, GLFW_KEY_D)) rot_value += 1.5;
        else if (glfwGetKey(window, GLFW_KEY_LEFT)) rot_value -= 1.5;
        else if (glfwGetKey(window, GLFW_KEY_RIGHT)) rot_value += 1.5;
